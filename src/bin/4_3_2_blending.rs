...
use learnopengl::ecs::components::{TextureInfo, Transform, Transparent};
...
pub fn main() -> Result<(), String> {
    ...
    let grass = build_plane(-0.5f32, 0.5f32, 1f32, vec![
        TextureInfo {
            id: 0,
            texture_type: TextureType::Diffuse,
            path: format!("{}/resource/window.png", env!("CARGO_MANIFEST_DIR")),
        }
    ]);
    for position in vec![
        Vector3::new(-1f32, 0f32, 0.48f32),
        Vector3::new(1.5f32, 0f32, 1.5f32),
        Vector3::new(0f32, 0f32, -2f32),
        Vector3::new(-0.8f32, 0f32, -3f32),
        Vector3::new(0.8f32, 0f32, -4.8f32),
    ] {
        let e = game.spawn_mesh(&grass, Transform {
            position,
            rotation: Rotation3::from_euler_angles(std::f32::consts::PI/2f32, 0f32, 0f32),
            scale: Vector3::new(1f32, 1f32, 1f32),
        })?;
        game.add_to(e, Transparent{})?;
    }
