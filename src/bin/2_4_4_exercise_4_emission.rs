...
pub fn main() -> Result<(), String> {
    ...
    let program = Program::new(vec![
        ...
        shader_loader.load(ShaderType::Fragment, "09.3-emission.glsl").unwrap(),
    ])?;
    ...
    {
        let data = include_bytes!("../../resource/container2_specular_colored.png.raw");
        specular_texture.set_image_2d(512, 512, data);
        specular_texture.generate_mipmap();
    }

    let emission_texture = Texture::new(TextureType::Texture2D);
    emission_texture.bind(gl::TEXTURE2);
    emission_texture.set_parameter(gl::TEXTURE_WRAP_S, gl::REPEAT);
    emission_texture.set_parameter(gl::TEXTURE_WRAP_T, gl::REPEAT);
    emission_texture.set_parameter(gl::TEXTURE_MIN_FILTER, gl::LINEAR_MIPMAP_LINEAR);
    emission_texture.set_parameter(gl::TEXTURE_MAG_FILTER, gl::LINEAR);
    {
        let data = include_bytes!("../../resource/matrix.jpeg.raw");
        emission_texture.set_image_2d(512, 512, data);
        emission_texture.generate_mipmap();
    }
    ...
    program.set_uniform_i1("material.emission", 2);
