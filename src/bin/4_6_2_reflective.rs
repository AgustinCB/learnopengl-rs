...
impl ReflectionSystem {
    fn new(camera: Rc<RefCell<Camera>>) -> Result<ReflectionSystem, String> {
        Ok(ReflectionSystem {
            camera,
            program: Program::new(vec![
                Shader::new(ShaderType::Vertex as _, include_str!("shaders/16.2-reflective_vertex.glsl"))?,
                Shader::new(ShaderType::Fragment as _, include_str!("shaders/16.2-reflective_fragment.glsl"))?,
            ])?,
        })
    }
}
...
pub fn main() -> Result<(), String> {
    ...
    game.add_to(e, Reflect)?;
    game.play_with_fps_camera(vec![
        Box::new(ReflectionSystem::new(game.camera())?),
    ])?;
    Ok(())
}