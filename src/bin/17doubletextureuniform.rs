...
const FRAGMENT_SHADER: &'static str = include_str!("shaders/03.3.4-twotextures.glsl");
const VERTICES: [f32; 32] = [
    0.5f32,  0.5, 0.0,   1.0, 0.0, 0.0,   1.0, 1.0,
...
pub fn main() -> Result<(), String> {
    ...
    VertexArray::set_vertex_attrib_with_padding::<f32>(gl::FLOAT, 0, 8, 3, 0, false);
    VertexArray::set_vertex_attrib_with_padding::<f32>(gl::FLOAT, 1, 8, 3, 3, false);
    VertexArray::set_vertex_attrib_with_padding::<f32>(gl::FLOAT, 2, 8, 2, 6, false);
    ...
    let mut current_blend = 0.2;
    program.set_uniform_f1("blend", current_blend);
        ...
        for event in event_pump.poll_iter() {
            match event {
                ...
                Event::KeyDown {
                    ...
                    current_blend = (current_blend + 0.01).clamp(0.0, 1.0);
                    program.set_uniform_f1("blend", current_blend);
                }
                Event::KeyDown {
